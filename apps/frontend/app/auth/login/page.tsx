...
// Uses Google Identity Services (GIS) directly for credential (id_token) flow
...
function GoogleSignInButton({ loading, label, onCredential, onError }: GoogleSignInButtonProps) {
  const callbackRef = useRef<((response: { credential: string }) => void) | null>(null);

  // Register callback on each render so it always has latest props
  callbackRef.current = (response: { credential: string }) => {
    if (response.credential) {
      onCredential(response.credential);
    } else {
      onError('Google login muvaffaqiyatsiz tugadi.');
    }
  };
  ...
  const handleClick = () => {
    ...
    if (!google?.accounts?.id) {
      onError("Google xizmati yuklanmadi. Sahifani yangilang.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        /* Google coloured SVG */
        <svg width="20" height="20" viewBox="0 0 48 48" className="shrink-0 relative z-10" aria-hidden>
...
export default function LoginPage() {
  ...
  // Watch for needsUsername transition false → true
  useEffect(() => {
  ...
  const handleGoogleCredential = async (idToken: string) => {
    ...
    try {
      ...
      // If needsUsername, the useEffect above will open the modal
  ...
  return (
    <>
      ...
      {/* Setup Profile Modal (shown when needsUsername is true after Google login) */}
      <AnimatePresence>
