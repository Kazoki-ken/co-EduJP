...
async function doDeploy() {
  ...
  try {
    ...
    console.log("Deploying DB & building backend...");
    const r1 = await ssh.execCommand('npm install && npx prisma migrate deploy && npm run build', { cwd: '/var/www/vocabjp/apps/backend' });
    console.log(r1.stdout);
    if(r1.stderr) console.error(r1.stderr);

    console.log("Building frontend...");
    const r2 = await ssh.execCommand('npm install && npm run build', { cwd: '/var/www/vocabjp/apps/frontend' });
